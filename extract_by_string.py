...
#
# list strings to search for in header lines and name to use in filenames
# left string is the search string, right string is the output file tag
#
...
#=====================
    ...
    #
    print '====================================================================='
    print ' extract_by_string.py, v.1.01, written by Phil Wilmarth, OHSU, 2009.'
    print '====================================================================='
    #
    ...
    #
    string_list = string_dict.items()
    string_list.sort()
    for obj in write:
        print >>obj, '...extracting entries containing these strings:'
        for i, t in enumerate(string_list):
            print >>obj, '......(%s) string "%s" to file ending in "%s"' % \
                  (i+1, t[0], t[1])
    #
    ...
    prot_read = 0
    for obj in write:
        print >>obj, '...reading %s and extracting entries...' % (db_name,)
    while x.readNextProtein(prot, check_for_errs=False):
        ...
        for pattern in string_dict.keys():
            ...
            for head in header.split(chr(01)):  # check each header for matches
                if new_pattern in head:
                    name = string_dict[pattern]
                    name_header = written.get(name, '')
                    if name_header:
                        name_header = name_header + chr(01) + head
    ...
    #
    for obj in write:
        print >>obj, '...%s protein entries in %s' % (prot_read, db_name)
        strings = string_count.keys()
        strings.sort()
        for i, string in enumerate(strings):
            print >>obj, '......(%s) pattern "%s" was found in %s proteins' % \
                  (i+1, string, string_count[string])
        print >>obj, '...output file summaries...'
        names = string_files.keys()
        names.sort()
        for i, name in enumerate(names):
            temp = base_name.replace('.fasta', '_'+name+'.fasta')
            print >>obj, '......(%s) %s proteins extracted and written to %s' % \
                  (i+1, name_count[name], temp)
    #
